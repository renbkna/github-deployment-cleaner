...
export default function GitHubDeployments() {
  ...
  const markInactive = async (id: string) => {
    try {
      const response = await fetch(`/api/deployments/${id}/mark_inactive`, {
        method: "POST",
      })
      if (!response.ok) {
      ...
      <div className="max-w-4xl mx-auto">
        <Card className="bg-white shadow-xl rounded-lg overflow-hidden">
          ...
          {messages.map((msg, index) => (
            <div
              key={index}
              className={`bg-${msg.category === "success" ? "green" : "red"}-100 border-l-4 border-${msg.category === "success" ? "green" : "red"}-500 text-${msg.category === "success" ? "green" : "red"}-700 p-4 mb-4`}
              role="alert"
            >
